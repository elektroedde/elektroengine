...
struct Rectangle: Transformable {
    ...
    init(device: MTLDevice) {
        ...
        for i in 0..<nodes {
            for j in 0..<nodes {
                vertices.append(Vertex(x: -1 + Float(2*j)/Float(nodes), y: -1 + Float(2*i)/Float(nodes), z: 0))
            }
        }
        ...
        for i in (nodes*nodes-nodes)..<nodes*nodes {
            dirichlet0.append(i)
        }
        ...
        femValues = Rectangle.solve(vertices: vertices, indices: indices, dirichlet1: dirichlet1, dirichlet0: dirichlet0)
...
extension Rectangle {
    static func solve(vertices: [Vertex],
        ...
        for element in 0..<M {
            ...
            let i: Int = Int(indices[element][0])
            let j: Int = Int(indices[element][1])
            let k: Int = Int(indices[element][2])
            let nodes = [i, j, k]
